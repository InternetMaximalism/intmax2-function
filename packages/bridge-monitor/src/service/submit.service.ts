...
export const submitTransaction = async (params: CallParams) => {
  ...
  for (let attempt = 0; attempt < TRANSACTION_MAX_RETRIES; attempt++) {
    try {
      ...
      const { transactionHash } = await submitWithRetry(params, multiplier, retryOptions);
...
export const submitWithRetry = async (
  ...
  const contract = MainnetBridgeOApp__factory.connect(contractCallParams.contractAddress, signer);
  const ethersTxOptions = getEthersTxOptions(contractCallParams, contractCallOptions ?? {});
  const callArgs = [...contractCallParams.args, ethersTxOptions];
